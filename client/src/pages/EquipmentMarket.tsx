...
export default function EquipmentMarket() {
  ...
  // Farmers can buy equipment, vendors can sell equipment, admins have full access
  useEffect(() => {
    if (!isLoading && user) {
      // Allow farmers, vendors and admins to access the equipment market
      if (user.userType !== 'farmer' && user.userType !== 'vendor' && user.role !== 'admin') {
        toast({
          title: "Access Restricted",
          description: "Only farmers and vendors can access the equipment market. Please contact support if you need access.",
          variant: "destructive"
        });
        setLocation("/");
      }
